...
// Prodotta dallo scheduled trigger decanter.js: un work item fermo troppo a
// lungo in uno stato. Non è anti-farming, ma finisce nella stessa coda del TL.
export const FLAG_DECANTER = 'DECANTER';
...
// `dettaglio` è un testo libero opzionale (lo usa il decanter per descrivere lo
// stato e i giorni). Le segnalazioni anti-farming lo lasciano vuoto.
export const aggiungiSegnalazione = async ({ issueKey, accountId, nome, flags, secondiInProgress, dettaglio }) => {
  ...
  const nuova = {
    ...
    secondiInProgress: secondiInProgress === null || secondiInProgress === undefined ? -1 : secondiInProgress,
    // KVS non accetta null: stringa vuota come "nessun dettaglio"
    dettaglio: dettaglio || '',
    data: Date.now(),
    risolta: false,
  };
