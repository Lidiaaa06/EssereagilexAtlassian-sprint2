...
function App() {
  ...
  const [activeAlertDays, setActiveAlertDays] = useState(null);
  ...
  useEffect(() => {
    if (isFinished || activeAlertDays !== null) {
      return;
    }

    const currentAlertDay = alertDays.find((days) => remaining.days === days);

    if (!currentAlertDay) {
      return;
    }

    const alertKey = getAlertKey(seasonEnd, currentAlertDay);
    const alreadyShown = window.localStorage.getItem(alertKey) === 'shown';

    if (!alreadyShown) {
      setActiveAlertDays(currentAlertDay);
    }
  }, [activeAlertDays, isFinished, remaining.days, seasonEnd]);

  function closeAlert() {
    if (activeAlertDays === null) {
      return;
    }

    const alertKey = getAlertKey(seasonEnd, activeAlertDays);
    window.localStorage.setItem(alertKey, 'shown');
    setActiveAlertDays(null);
  }
  ...
  return (
    <>
      ...
      <ModalTransition>
        {activeAlertDays !== null && (
          <Modal onClose={closeAlert}>
            <ModalHeader>
              <ModalTitle>Fine stagione in arrivo</ModalTitle>
            </ModalHeader>

            <ModalBody>
              <Stack space="space.150">
                <Text>
                  Mancano {activeAlertDays}{' '}
                  {activeAlertDays === 1 ? 'giorno' : 'giorni'} alla fine della stagione.
                </Text>
                <Text>La stagione termina il {formatDate(seasonEnd)}.</Text>
              </Stack>
            </ModalBody>

            <ModalFooter>
              <Button appearance="primary" onClick={closeAlert}>
                Ho capito
              </Button>
            </ModalFooter>
          </Modal>
